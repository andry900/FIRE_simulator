...
def render() -> dict:
    ...
    with st.sidebar:
        ...
        # ── Cash flow ────────────────────────────────────────────────────────
        with st.expander("💰 Cash flow mensile", expanded=False):
        ...
        with st.expander("📈 Rendimento & inflazione", expanded=False):
        ...
        # ── Spese post-FIRE ──────────────────────────────────────────────────
        with st.expander("🧾 Spese post-FIRE", expanded=False):
        ...
        with st.expander("🏖️ Simulazione", expanded=False):
            ...
            pension_access_default = max(57, min(pension_access_default, 75))
            pension_access_age = st.number_input(
                "Età pensione INPS", value=pension_access_default,
                step=1, min_value=57, max_value=75,
            )
        ...
        with st.expander("🟣 INPS", expanded=False):
            ...
            inps_fill_missing_years = st.checkbox(
                ...
                value=bool(p.get("inps_fill_missing_years", 0)),
                help="Se attivo, il modello continua ad aggiungere contributi INPS dopo FIRE finche non raggiungi 20 anni.",
            )
            inps_gross_factor = st.slider(
                "Fattore prudenziale pensione INPS lorda (%)", 30.0, 100.0,
                float(p.get("inps_gross_factor", 1.0) * 100), 1.0,
            ) / 100

            # Proiezione INPS deterministica con rivalutazione ANNUALE.
            inps_montante_proj = inps_montante_current
            inps_years_proj = max(float(inps_years_contributed_current), 0.0)
            ...
            for m in range(months_to_inps):
                ...
                if (
                    ...
                    and inps_fill_missing_years
                    and age_t < float(pension_access_age)
                    and inps_years_proj < 20.0
                ):
            ...
            inps_eligible = inps_years_proj >= 20.0
            inps_proj_pension = (
                inps_montante_proj * inps_coeff * inps_gross_factor
                if inps_eligible
                else 0.0
            )
            ...
            st.caption(
                ...
                f"× fattore prudenziale {inps_gross_factor * 100:.0f}%."
            )
            if inps_eligible:
                st.caption(
                    f"Anni contributivi stimati a {pension_access_age} anni: {inps_years_proj:.1f} (requisito 20 anni OK)."
                )
                st.caption(
                ...
                st.warning(
                    f"Anni contributivi stimati a {pension_access_age} anni: {inps_years_proj:.1f}. "
                    "Requisito minimo 20 anni NON raggiunto: pensione INPS non erogabile nel modello."
                )

        # ── Tassazione prelievi ──────────────────────────────────────────────
        with st.expander("🧾 Tassazione prelievi", expanded=False):
            initial_gain_pct = st.slider(
                "Plusvalenza attuale sul portafoglio (%)", 0.0, 80.0,
                float(p.get("initial_gain_pct", 0.30) * 100), 0.1,
            ) / 100
        ...
        # ── Casa vs Affitto ──────────────────────────────────────────────────
        with st.expander("🏠 Casa vs Affitto", expanded=False):
        ...
        # ── Rischio di mercato ───────────────────────────────────────────────
        with st.expander("🌪️ Rischio di mercato", expanded=False):
        ...
        if st.button("💾 Salva parametri", use_container_width=True):
            save_params({
                ...
                "fonte_access_age":              float(fonte_access_age),
    ...
    return dict(
        ...
        inps_montante_revaluation_rate=inps_montante_revaluation_rate,
        inps_years_contributed_current=inps_years_contributed_current,
        inps_fill_missing_years=bool(inps_fill_missing_years),
        inps_gross_factor=inps_gross_factor,
        inps_coefficient_haircut=inps_coefficient_haircut,
        initial_gain_pct=initial_gain_pct,
        state_bond_share=state_bond_share,
